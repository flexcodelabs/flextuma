...
public interface WhatsAppWebhookConfigRepository extends BaseRepository<WhatsAppWebhookConfig, UUID>, JpaSpecificationExecutor<WhatsAppWebhookConfig> {
    ...
    // createdBy is fetched eagerly here: the webhook controller reads config.getCreatedBy()
    // (to stamp ownership on inbound messages/relay rows) after this lookup's own transaction
    // has already closed (spring.jpa.open-in-view=false, no @Transactional on the controller),
    // so a lazy load at that point throws LazyInitializationException.
    @Query("SELECT c FROM WhatsAppWebhookConfig c JOIN FETCH c.createdBy WHERE c.phoneNumberId = :phoneNumberId AND c.active = true")
    ...
    @Query("SELECT c FROM WhatsAppWebhookConfig c JOIN FETCH c.createdBy WHERE c.callbackToken = :callbackToken AND c.active = true")
